...
class UnregisteredOrderCleanupService:
    ...
    async def _perform_cleanup(self):
        ...
        try:
            ...
            unregistered_orders = self._find_unregistered_orders(exchange_orders, oms_orders)
            ...
            logger.error(
                "Failed to perform cleanup cycle",
                error=str(e),
                exc_info=True
            )
            raise
    ...
    def _find_unregistered_orders(
        ...
        for exchange_order in exchange_orders:
            ...
            exchange_order_id = str(exchange_order.get("order_id") or exchange_order.get("id", ""))
            ...
            if exchange_order_id and exchange_order_id not in registered_ids:
                ...
                order_time = exchange_order.get("created_at", exchange_order.get("timestamp", 0))
        ...
        for i, order in enumerate(unregistered_orders):
            ...
            if (i + 1) % batch_size == 0:
                logger.info(
                    ...
                    estimated_time_remaining_minutes=((len(unregistered_orders) - i - 1) * 2.5) / 60
                )
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "last_cleanup_age_seconds": int(time.time() - self.last_cleanup_time) if self.last_cleanup_time > 0 else None,
            "interval_ms": settings.system.cleanup_check_interval_ms,
