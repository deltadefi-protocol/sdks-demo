...
from bot.quote import BookTicker, Quote
...
class PersistentQuote:
    ...
    @classmethod
    def from_quote(cls, quote: Quote, strategy: QuoteStrategy = QuoteStrategy.MARKET_MAKING) -> 'PersistentQuote':
        ...
        return cls(
            ...
            mid_price=Decimal(str((quote.bid_price + quote.ask_price) / 2)) if quote.bid_price and quote.ask_price else None,
    ...
    def has_bid(self) -> bool:
        ...
        return self.bid_price is not None and self.bid_qty is not None and self.bid_qty > 0
    
    @property
    def has_ask(self) -> bool:
        """Check if quote has valid ask"""
        return self.ask_price is not None and self.ask_qty is not None and self.ask_qty > 0
...
class QuoteRepository:
    ...
    async def save_quote(self, quote: PersistentQuote) -> int:
        ...
        async with db_manager.get_connection() as conn:
            cursor = await conn.execute(query, (
                quote.quote_id, quote.timestamp, quote.symbol_src, quote.symbol_dst,
                float(quote.source_bid_price), float(quote.source_bid_qty),
                float(quote.source_ask_price), float(quote.source_ask_qty),
    ...
    async def update_quote_status(self, quote_id: str, status: QuoteStatus, **kwargs) -> None:
        """Update quote status and optional fields"""
        set_clause = "status = ?, updated_at = ?"
        params = [status, time.time()]
        
        for field, value in kwargs.items():
            if field in ['bid_order_id', 'ask_order_id', 'expires_at']:
                set_clause += f", {field} = ?"
                params.append(value)
    ...
    async def get_quote(self, quote_id: str) -> Optional[PersistentQuote]:
        """Get quote by ID"""
    ...
    async def expire_old_quotes(self, symbol_dst: Optional[str] = None) -> int:
...
class QuoteToOrderPipeline:
    ...
    async def process_quote(self, quote: Quote, strategy: QuoteStrategy = QuoteStrategy.MARKET_MAKING) -> PersistentQuote:
        """
    ...
    async def _persist_quote(self, quote: PersistentQuote):
        ...
        try:
            ...
            logger.error("Failed to persist quote", quote_id=quote.quote_id, error=str(e))
            raise
    ...
    async def _generate_orders(self, quote: PersistentQuote):
        ...
        try:
            ...
            for order in orders_created:
                try:
                    await self.oms.cancel_order(order.order_id, "Quote processing failed")
                except Exception:
    ...
    async def _cancel_quote(self, quote: PersistentQuote):
        ...
        try:
            ...
            logger.error("Error cancelling quote", quote_id=quote.quote_id, error=str(e))
    ...
    async def _on_order_update(self, order: OMSOrder):
        ...
        if order.is_complete:
            ...
            if quote.bid_order_id:
                ...
                all_complete = all_complete and (bid_order is None or bid_order.is_complete)
            if quote.ask_order_id:
                ask_order = self.oms.get_order(quote.ask_order_id)
                all_complete = all_complete and (ask_order is None or ask_order.is_complete)
    ...
    async def _notify_quote_callbacks(self, quote: PersistentQuote):
        ...
        for callback in self.quote_callbacks:
            try:
                ...
                logger.error("Error in quote callback", callback=callback.__name__, error=str(e))
    ...
    async def _notify_order_callbacks(self, order: OMSOrder):
        ...
        for callback in self.order_callbacks:
            try:
                ...
                logger.error("Error in order callback", callback=callback.__name__, error=str(e))