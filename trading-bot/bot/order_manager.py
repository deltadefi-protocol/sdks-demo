...
class DataAggregator:
    ...
    def __init__(self, 
                 ...
                 price_threshold: float = 0.001):  # 0.1% price change threshold
    ...
    async def add_data(self, data: MarketData) -> Optional[MarketData]:
        """
...
class OrderManager:
    ...
    async def _process_next_order(self):
        ...
        if await self.rate_limiter.acquire():
            async with self._order_lock:
                if self.pending_orders and self.pending_orders[0].order_id == order.order_id:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_order": self.pending_orders[0].order_id if self.pending_orders else None
        }