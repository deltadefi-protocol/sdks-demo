    ...
    def check_risk(self, order: OMSOrder, position: Position) -> tuple[bool, List[str]]:
        ...
        if position:
            ...
            if new_position_size > Decimal(str(settings.risk.max_position_size)):
                violations.append(f"Position size would exceed limit: {new_position_size} > {settings.risk.max_position_size}")
        ...
        if position and hasattr(settings.trading, 'max_skew'):
            if abs(position.quantity) > Decimal(str(settings.trading.max_skew)):
                violations.append(f"Position skew too large: {position.quantity}")
        
        # Minimum quantity check
        if hasattr(settings.trading, 'min_quote_size'):
            if order.quantity < Decimal(str(settings.trading.min_quote_size)):
...
class OrderManagementSystem:
    ...
    def __init__(self):
        ...
        self.valid_transitions = {
            ...
            OrderState.PENDING: [OrderState.WORKING, OrderState.REJECTED, OrderState.FAILED],
            OrderState.WORKING: [OrderState.FILLED, OrderState.CANCELLED, OrderState.REJECTED],
    ...
    async def add_fill(
        ...
        """
        if order_id not in self.orders:
            logger.warning("Fill for unknown order", order_id=order_id, trade_id=trade_id)
            return
        ...
        # Update position
        await self._update_position(order.symbol, order.side, fill_quantity, fill_price, fee)
    ...
    async def cancel_order(self, order_id: str, reason: str = "User requested"):
        ...
        if order.is_complete:
            logger.warning("Attempted to cancel completed order", order_id=order_id, state=order.state)
            return
        
        await self._transition_order_state(order, OrderState.CANCELLED, error_message=reason)
        self.risk_manager.open_order_count -= 1
        
        logger.info("Order cancelled", order_id=order_id, reason=reason)
    
    async def _transition_order_state(self, order: OMSOrder, new_state: OrderState, **kwargs):
    ...
    async def _update_position(
        ...
        elif (position.quantity > 0 and side == OrderSide.BUY) or (position.quantity < 0 and side == OrderSide.SELL):
            # Adding to position
            old_notional = position.quantity * position.avg_price
            new_notional = old_notional + (quantity_delta * price)
            position.avg_price = abs(new_notional / (position.quantity + quantity_delta))
        else:
            # Reducing position - realize PnL
            fill_pnl = quantity * (price - position.avg_price) * (-1 if side == OrderSide.SELL else 1)
    ...
    def get_all_orders(self, symbol: Optional[str] = None, state: Optional[OrderState] = None) -> List[OMSOrder]:
    ...
    def get_open_orders(self, symbol: Optional[str] = None) -> List[OMSOrder]:
