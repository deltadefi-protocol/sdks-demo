...
class TradingBot:
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info(
                ...
                deltadefi_ready=self.deltadefi_client._operation_key_loaded
    ...
    async def _initialize_components(self):
        ...
        self.cleanup_service = UnregisteredOrderCleanupService(
            deltadefi_client=self.deltadefi_client, oms=self.oms
        )
    ...
    async def _start_services(self):
        ...
        try:
            ...
            logger.warning(
                "Account manager failed to start, continuing with reduced functionality",
                error=str(e),
            )
    ...
    def _setup_callbacks(self):
        ...
        if self.account_manager:
            self.account_manager.fill_reconciler.add_fill_callback(
    ...
    async def _process_binance_message(self, message: dict[str, Any]):
        ...
        try:
            ...
            if quote:
                ...
                try:
                    ...
                    logger.info(
                        ...
                        bid_price=float(processed_quote.bid_price)
                        if processed_quote.bid_price
                        else None,
                        ask_price=float(processed_quote.ask_price)
                        if processed_quote.ask_price
                        else None,
            ...
            logger.error(
                "Error processing Binance message", error=str(e), message=message
            )
    ...
    async def _on_fill_received(self, fill):
        ...
        try:
            ...
            logger.error(
                "Failed to update OMS with fill", fill_id=fill.fill_id, error=str(e)
            )
        ...
        try:
            ...
            while self.running:
                ...
                if current_time - last_cleanup > 10:
                    if self.quote_pipeline:
                        expired_count = (
                            await self.quote_pipeline.cleanup_expired_quotes()
                        )
                        if expired_count > 0:
                            logger.debug(
                                "Background cleanup removed expired quotes",
                                count=expired_count,
                            )
    ...
    async def _log_status(self):
        ...
        status = {
            ...
            "last_quote_age_seconds": int(time.time() - self.last_quote_time)
